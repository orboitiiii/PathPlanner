...
YawProfile buildYawProfile({
  ...
  for (int i=0;i<waypointS.length;i++){
    ...
    int idx = 0;
    while(idx < n-1 && sSample[idx] < waypointS[i]) idx++;
    idx = idx.clamp(0, n-1);
    ...
    final m = (waypointThetaDeg[i] == null) ? 0.0 /* we avoid forcing slope to reduce overshoot */ : 0.0;
    mLoc.add(m);
  }
  ...
  for (int k=0;k<n;k++){
    final s = sSample[k];
    while(seg < waypointS.length-2 && s > waypointS[seg+1]) seg++;
    ...
    final m1 = mLoc[seg+1];
    if (s1 - s0 < 1e-6){
      theta[k] = th0; dth[k]=0; ddth[k]=0; continue;
    }
    ...
    final h11 = (u*u*u - u*u);
    final th = h00*th0 + (s1-s0)*h10*m0 + h01*th1 + (s1-s0)*h11*m1;
    // unwrap w.r.t tangent to avoid jumps
    theta[k] = _wrapPi(th);
    final d_u_ds = 1.0/(s1 - s0);
    final dh00 = (6*u*u - 6*u) * d_u_ds;
    final dh10 = (3*u*u - 4*u + 1) * d_u_ds;
    final dh01 = (-6*u*u + 6*u) * d_u_ds;
    final dh11 = (3*u*u - 2*u) * d_u_ds;
    dth[k] = dh00*th0 + dh10*(s1-s0)*m0 + dh01*th1 + dh11*(s1-s0)*m1;
