...
SwerveLimitResult swerveVelocityBounds({
  ...
  required List<double> kappa,   // 1/m (path curvature, for lateral acceleration only)
  ...
  final halfX = trackWidth/2.0;
  final halfY = wheelBase/2.0;
  final mods = [
    [ halfX,  halfY], // FL (x to left? using x=left? We'll adopt x=left??? Here we define x = right??)
    [ halfX, -halfY], // FR
    [-halfX,  halfY], // RL
    [-halfX, -halfY], // RR
  ];
  for (int i=0;i<n;i++){
    final delta = _wrapPi(theta[i] - tangent[i]); // translational direction in robot frame
    ...
    final limitYawRate = (dtheta_ds[i].abs() > 1e-9) ? (yawRateMax / dtheta_ds[i].abs()) : vMaxChassis;
