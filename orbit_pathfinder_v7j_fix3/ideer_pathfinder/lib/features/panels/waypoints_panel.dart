...
class WaypointsPanel extends StatefulWidget {
  ...
  final VoidCallback onAddPassThrough; // 新增：過點不停
  ...
  final void Function(int passIdx, double x, double y, double? thetaDeg)
      onUpdatePass;
  ...
  const WaypointsPanel({
    ...
    required this.onAddPassThrough,
...
class _WaypointsPanelState extends State<WaypointsPanel> {
  ...
  void _initOrUpdateControllers({required bool fullRebuild}) {
    ...
    // 過濾 pass 和 passThrough 類型的航點
    final passes = widget.waypoints
        .where((w) =>
            w.kind == WaypointKind.pass || w.kind == WaypointKind.passThrough)
        .toList();
    if (pxs.length < passes.length) {
      for (int i = pxs.length; i < passes.length; i++) {
        ...
        pts.add(TextEditingController(
            text: (passes[i].thetaDeg ?? 0.0).toStringAsFixed(1)));
      }
  ...
  Widget _xytheta(String title, TextEditingController cx,
      TextEditingController cy, TextEditingController ct,
      {void Function()? onDel}) {
    return Padding(
          ...
          Row(children: [
            ...
            SizedBox(
              ...
              child: TextField(
                  ...
                  keyboardType: TextInputType.number,
                  decoration: const InputDecoration(
                      isDense: true, labelText: 'θ (deg)')),
            ),
            if (onDel != null) ...[
              const SizedBox(width: 8),
              IconButton(
                  onPressed: onDel, icon: const Icon(Icons.delete_outline))
            ],
          ]),
        ],
      ),
  ...
  Widget build(BuildContext context) {
    final passList = widget.waypoints
        .asMap()
        .entries
        .where((e) =>
            e.value.kind == WaypointKind.pass ||
            e.value.kind == WaypointKind.passThrough)
        .toList();
    final n = passList.length;
    return Column(
      crossAxisAlignment: CrossAxisAlignment.start,
      children: [
        const Text("起點 / 終點（輸入座標與角度，也可拖曳）",
        ...
        Row(children: [
          ...
          FilledButton.icon(
              onPressed: () {
                final x = double.tryParse(sx.text),
                    y = double.tryParse(sy.text),
                    t = double.tryParse(st.text);
                if (x != null && y != null) widget.onUpdateStart(x, y, t);
              },
              icon: const Icon(Icons.save),
              label: const Text("套用起點")),
        ]),
        ...
        Row(children: [
          ...
          FilledButton.icon(
              onPressed: () {
                final x = double.tryParse(ex.text),
                    y = double.tryParse(ey.text),
                    t = double.tryParse(et.text);
                if (x != null && y != null) widget.onUpdateEnd(x, y, t);
              },
              icon: const Icon(Icons.save),
              label: const Text("套用終點")),
        ]),
        ...
        Row(children: [
          ...
          IconButton(
              tooltip: "添加必經點（停車）",
              onPressed: widget.onAddPass,
              icon: const Icon(Icons.add_circle_outline,
                  color: Color(0xFFFFD54F))),
          IconButton(
              tooltip: "添加過點不停（不停車）",
              onPressed: widget.onAddPassThrough,
              icon: const Icon(Icons.add_circle_outline,
                  ...
                  children: [
                    ...
                    Container(
                      ...
                      decoration: BoxDecoration(
                        color: widget.waypoints[1 + i].kind ==
                                WaypointKind.passThrough
                            ? const Color(0xFF00BCD4).withOpacity(0.3)
                            : const Color(0xFFFFD54F).withOpacity(0.3),
                        borderRadius: BorderRadius.circular(4),
                      ),
                      child: Text(
                        widget.waypoints[1 + i].kind == WaypointKind.passThrough
                            ? "過點不停"
                            : "停車點",
                        style: TextStyle(
                          fontSize: 10,
                    ...
                    const SizedBox(width: 8),
                    Text(widget.waypoints[1 + i].label,
                        style: const TextStyle(color: Colors.white70)),
                  ],
                ),
                _xytheta("", pxs[i], pys[i], pts[i],
                    onDel: () => widget.onDeletePass(i)),
                Row(children: [
                  const Spacer(),
                  TextButton.icon(
                      onPressed: () {
                        ...
                        if (x != null && y != null)
                          widget.onUpdatePass(i, x, y, t);
                      },
                      icon: const Icon(Icons.save_as),
                      label: const Text("套用座標/角度")),
                ]),
                const Divider(),
              ],
            );
